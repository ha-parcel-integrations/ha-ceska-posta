"""Tests for Ceska Posta sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.ceska_posta.const import ParcelStatus
from custom_components.ceska_posta.sensor import (
    CeskaPostaAwaitingPickupSensor,
    CeskaPostaDeliveredParcelsSensor,
    CeskaPostaIncomingParcelsSensor,
    CeskaPostaLastUpdateSensor,
    CeskaPostaNextDeliverySensor,
    CeskaPostaParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = CeskaPostaIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_awaiting_pickup_counts_only_ready_pickup_point_parcels():
    ...
    sensor = CeskaPostaAwaitingPickupSensor(
...
def test_awaiting_pickup_zero_when_no_parcels():
    sensor = CeskaPostaAwaitingPickupSensor(_coordinator([]), _entry())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = CeskaPostaParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = CeskaPostaParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = CeskaPostaNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = CeskaPostaNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = CeskaPostaDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = CeskaPostaLastUpdateSensor(coordinator, _entry())
