"""pytest configuration for the Ceska Posta test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.ceska_posta`` loadable from config-flow / setup tests."""
    yield
...
def reset_one_shot_warnings():
    ...
    """
    from custom_components.ceska_posta import parcels

    parcels._unmapped_statuses_logged.clear()
    parcels._not_found_logged.clear()
    parcels._bg_anomaly_logged.clear()
    parcels._eta_populated_warned = False
    yield
