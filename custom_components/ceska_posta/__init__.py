"""Ceska Posta parcel tracker custom component for Home Assistant."""
...
from .coordinator import CeskaPostaCoordinator, _refresh_interval
...
@dataclass
class CeskaPostaData:
    """Runtime data attached to the Ceska Posta config entry."""

    client: CeskaPostaApiClient
    coordinator: CeskaPostaCoordinator


type CeskaPostaConfigEntry = ConfigEntry[CeskaPostaData]


async def async_setup_entry(hass: HomeAssistant, entry: CeskaPostaConfigEntry) -> bool:
    """Set up Ceska Posta from a config entry."""
    # No auth: Ceska Posta tracking is public, so the HA-managed session is fine.
    ...
    entry.runtime_data = CeskaPostaData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: CeskaPostaConfigEntry) -> bool:
    """Unload the Ceska Posta config entry."""
