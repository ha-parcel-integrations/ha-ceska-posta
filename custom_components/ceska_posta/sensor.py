"""Sensor platform for the Ceska Posta parcel tracker integration."""
...
from .coordinator import CeskaPostaCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: CeskaPostaConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Ceska Posta sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        CeskaPostaIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        CeskaPostaAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            CeskaPostaParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(CeskaPostaNextDeliverySensor(coordinator, entry))
    entities.append(CeskaPostaDeliveredParcelsSensor(coordinator, entry))
    entities.append(CeskaPostaLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class CeskaPostaAwaitingPickupSensor(
    CoordinatorEntity[CeskaPostaCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: CeskaPostaCoordinator, entry: ConfigEntry) -> None:
...
class CeskaPostaIncomingParcelsSensor(
    CoordinatorEntity[CeskaPostaCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: CeskaPostaCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                CeskaPostaParcelSensor(self.coordinator, self._entry, barcode)
...
class CeskaPostaParcelSensor(CoordinatorEntity[CeskaPostaCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Ceska Posta parcel."""
    ...
    def __init__(
        self, coordinator: CeskaPostaCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class CeskaPostaNextDeliverySensor(
    CoordinatorEntity[CeskaPostaCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: CeskaPostaCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class CeskaPostaDeliveredParcelsSensor(
    CoordinatorEntity[CeskaPostaCoordinator], SensorEntity
):
    """Recently delivered tracked Ceska Posta parcels."""
...
class CeskaPostaLastUpdateSensor(
    CoordinatorEntity[CeskaPostaCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Ceska Posta was last polled successfully."""
